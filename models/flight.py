...
def create_flight(flight_number, airline_id, aircraft_id, origin_airport_id, destination_airport_id, departure_datetime, arrival_datetime, economy_price, business_price, status):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO flights (flight_number, airline_id, aircraft_id, origin_airport_id, destination_airport_id, departure_datetime, arrival_datetime, economy_price, business_price, status) "
        "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
        (flight_number, airline_id, aircraft_id, origin_airport_id, destination_airport_id, departure_datetime, arrival_datetime, economy_price, business_price, status),
    )
    conn.commit()
    cursor.close()
    conn.close()
...
def update_flight(flight_id, flight_number, airline_id, aircraft_id, origin_airport_id, destination_airport_id, departure_datetime, arrival_datetime, economy_price, business_price, status):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE flights SET flight_number=%s, airline_id=%s, aircraft_id=%s, origin_airport_id=%s, destination_airport_id=%s, departure_datetime=%s, arrival_datetime=%s, economy_price=%s, business_price=%s, status=%s WHERE id=%s",
        (flight_number, airline_id, aircraft_id, origin_airport_id, destination_airport_id, departure_datetime, arrival_datetime, economy_price, business_price, status, flight_id),
    )
    conn.commit()
    cursor.close()
    conn.close()
...
def search_flights(origin_id=None, destination_id=None, departure_date=None, sort_by="departure_datetime", airline_id=None, min_price=None, max_price=None, time_of_day=None, passenger_count=1):
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
