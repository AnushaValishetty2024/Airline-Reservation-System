...
def create_airline(airline_name: str, airline_code: str, country: str, is_active: int = 1):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        """
...
def update_airline(airline_id: int, airline_name: str, airline_code: str, country: str, is_active: int):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE airlines SET airline_name = %s, airline_code = %s, country = %s, is_active = %s WHERE id = %s",
        (airline_name, airline_code, country, is_active, airline_id),
    )
    conn.commit()
    cursor.close()
    conn.close()
